...
class TransformationEvaluator:
    def __init__(self, gt_file, res_file, ply_file):
        ...
        self.gt_matrices = self._load_yaml(self.gt_file)
        self.res_matrices = self._load_yaml(self.res_file)
        self.object_points = self._load_ply(self.ply_file)
    ...
    def evaluate(self):
        ...
        for i in range(self.num_matrices):
            ...
            self.rotation_errors.append(self.compute_rotation_error(R_gt, R_pred))
            self.translation_errors.append(self.compute_translation_error(t_gt, t_pred))
            self.pose_errors.append(self.compute_pose_error(T_gt, T_pred))
            self.add_errors.append(self.compute_add(T_gt, T_pred))
        
        metrics = {
            "Rotation Error (deg)": np.mean(self.rotation_errors),
            "Translation Error (m)": np.mean(self.translation_errors),
            "Pose Error (Frobenius norm)": np.mean(self.pose_errors),
            "ADD (m)": np.mean(self.add_errors)
        }
        return metrics

    def plot_results(self):
        frames = np.arange(len(self.rotation_errors))
        ...
        axs[0].scatter(frames, self.rotation_errors, color="blue", alpha=0.5, label="Rotation Error")
        axs[0].scatter([i for i, e in enumerate(self.rotation_errors) if e > rot_threshold],
                       [e for e in self.rotation_errors if e > rot_threshold],
        ...
        axs[1].scatter(frames, self.translation_errors, color="orange", alpha=0.5, label="Translation Error")
        axs[1].scatter([i for i, e in enumerate(self.translation_errors) if e > trans_threshold],
                       [e for e in self.translation_errors if e > trans_threshold],
        ...
        axs[2].scatter(frames, self.pose_errors, color="green", alpha=0.5, label="Pose Error")
        axs[2].scatter([i for i, e in enumerate(self.pose_errors) if e > pose_threshold],
                       [e for e in self.pose_errors if e > pose_threshold],
        ...
        axs[3].scatter(frames, self.add_errors, color="purple", alpha=0.5, label="ADD Error")
        axs[3].scatter([i for i, e in enumerate(self.add_errors) if e > add_threshold],
                       [e for e in self.add_errors if e > add_threshold],
                       color="red", label="Outlier (>0.05m)")
        ...
        axs[0].plot(frames, self.rotation_errors, label="Rotation Error (deg)", color="blue")
        ...
        axs[1].plot(frames, self.translation_errors, label="Translation Error (m)", color="orange")
        ...
        axs[2].plot(frames, self.pose_errors, label="Pose Error (Frobenius norm)", color="green")
        ...
        axs[3].plot(frames, self.add_errors, label="ADD Error (m)", color="purple")
        ...
        axs[0].hist(self.rotation_errors, bins=50, color="blue", alpha=0.7)
        ...
        axs[1].hist(self.translation_errors, bins=50, color="orange", alpha=0.7)
        ...
        axs[2].hist(self.pose_errors, bins=50, color="green", alpha=0.7)
        ...
        axs[3].hist(self.add_errors, bins=50, color="purple", alpha=0.7)
