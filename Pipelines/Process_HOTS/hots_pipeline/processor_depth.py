...
from .config import REQUIRE_DEPTH_NPY
...
class DepthProcessor(ModalityProcessor):
    ...
    def save_to(self, base_name, output_path):
        ...
        if file_type is None:
            print(f"!!! No valid depth file found for {base_name} !!!")
            return

        if file_type == "png":
            # Convert PNG to NPY format if required
            if REQUIRE_DEPTH_NPY:
                print(f"!!! Skipping PNG depth for {base_name} (NPY required) !!!")
                return
            ...
            try:
                ...
                print(f"!!! Failed to process depth file {depth_file}: {str(e)} !!!")